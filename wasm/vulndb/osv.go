...
func newOSV() *osvClient { return &osvClient{} }
...
func (c *osvClient) batch(pkgs []Package) ([]bool, error) {
	...
	for start := 0; start < len(pkgs); start += osvBatchSize {
		...
		if err := postJSON(osvBase+"/v1/querybatch", body, &out); err != nil {
			return nil, err
		}
...
// bestimmten Version.
func (c *osvClient) query(p Package) ([]osvVuln, error) {
	body := map[string]any{
	...
	if err := postJSON(osvBase+"/v1/query", body, &out); err != nil {
		return nil, err
	}
...
func (c *osvClient) vuln(id string) (*osvVuln, error) {
	var v osvVuln
	if err := getJSON(osvBase+"/v1/vulns/"+id, nil, &v); err != nil {
		return nil, err
	}
	return &v, nil
}
