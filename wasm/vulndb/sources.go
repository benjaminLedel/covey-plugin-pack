...
func fetchGHSA(id string) (*ghsaAdvisory, error) {
	var a ghsaAdvisory
	if err := getJSON(ghsaBase+"/advisories/"+url.PathEscape(id), ghsaHeader(), &a); err != nil {
		return nil, err
	}
	return &a, nil
}
...
func fetchGHSAByCVE(cve string) (*ghsaAdvisory, error) {
	var list []ghsaAdvisory
	u := ghsaBase + "/advisories?cve_id=" + url.QueryEscape(cve)
	if err := getJSON(u, ghsaHeader(), &list); err != nil {
		return nil, err
	}
	if len(list) == 0 {
...
// fetchNVD fetches score and vector for a CVE.
//
// There is no API key any more, and there cannot be one here: the host attaches
// a brokered credential only to the system an organisation pointed the plugin
// at, and this plugin points at nobody — every source is public and declared.
// So NVD answers at its anonymous limit of 5 requests per 30 seconds. That
// costs the advisory action nothing it cannot survive: a 429 comes back as a
// note beside the merged result, the same as any other source that does not
// answer.
func fetchNVD(cve string) (score float64, severity, vector string, err error) {
	...
	if err := getJSON(u, header, &out); err != nil {
		return 0, "", "", err
	}
...
func fetchVersions(ecosystem, name string) (VersionInfo, error) {
	...
	case EcosystemNPM:
		...
		if err := getJSON(npmBase+"/"+npmPath(name), header, &out); err != nil {
			return info, err
		}
	...
	case EcosystemPackagist:
		...
		if err := getJSON(packagistAPI+"/p2/"+name+".json", nil, &out); err != nil {
			return info, err
		}
	...
	case EcosystemPub:
		...
		if err := getJSON(pubBase+"/api/packages/"+url.PathEscape(name), nil, &out); err != nil {
			return info, err
		}
